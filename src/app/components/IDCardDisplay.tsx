...
import type { BackSideText, Branch, Template } from '../utils/templateData';
...
interface IDCardDisplayProps {
  ...
  side: 'front' | 'back';
  scale?: number;
  template?: Template; // Optional template prop to override default ACC design
}
 ...
 * Can optionally accept a template prop to render different template designs
 */
export function IDCardDisplay({ employee, side, scale = 1, template }: IDCardDisplayProps) {
  ...
  // FRONT SIDE
  // Get template-specific styling or use ACC defaults
  const cardBackground = template?.front.backgroundColor || '#FFFFFF';
  const nameColor = template?.front.nameStyle.color || '#213876';
  const employeeIdColor = template?.front.employeeIdStyle.color || '#0f172a';
  const textColor = template?.colors.text || '#0f172a';
  const accentColor = template?.colors.accent || '#0066CC';
  
  // Detect if background is a gradient
  ...
  return (
    ...
    >
      {/* Render accent elements if template provided */}
      {template?.front.accentElements.map((element, index) => (
        <div
          key={index}
          className="absolute"
          style={{
