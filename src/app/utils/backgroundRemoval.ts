 ...
 * 7. ✅ FACE-AWARE PROTECTION (preserves face during cleanup)
 * 8. ✅ VALIDATION SYSTEM (checks if face is still intact after removal)
 * 
 * @param file - The image file to process
 * @param faceBox - Optional face bounding box to protect during processing
 * @returns Promise with {file: File, blob: Blob, hadTransparency: boolean, facePreserved: boolean}
 */

// Import face detection
import { detectFaceForCropping, FaceBox } from './photoCropper';
...
export async function removeImageBackground(
  ...
  // 🔍 STEP 0: Detect face if not provided (for protection during cleanup)
  if (!faceBox) {
    console.log('🔍 Step 0a: Detecting face for protection...');
    try {
      ...
      console.warn('Face detection failed, proceeding without face protection:', e);
    }
  }

  // 🔍 STEP 0b: Check if image already has transparent background
  console.log('🔍 Step 0b: Checking for existing transparency...');
  ...
  if (hasTransparency) {
    ...
    return { file, blob: await file.arrayBuffer() as Blob, hadTransparency, facePreserved: true };
  }
  ...
  try {
    ...
    let refinedBlob = await smartCleanup(blob, faceBox);
    ...
    return { file: processedFile, blob: refinedBlob, hadTransparency, facePreserved: true };
        ...
        canvas.toBlob(
          (blob) => {
            ...
            if (blob) {
              ...
              resolve({ file: pngFile, blob, hadTransparency: true, facePreserved: true });
            } else {
...
async function removeBackgroundWithAPI(file: File, apiKey: string): Promise<{
  ...
  return {
    ...
    facePreserved: true, // remove.bg is professional and preserves faces
  };
}
